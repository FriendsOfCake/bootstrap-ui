...
class ColorModeHelper extends Helper
{
    ...
    public function toggle(array $options = []): string
    {
        ...
        $initJs = '(function(){'
            ...
            . 'var active=(window.BootstrapUIColorMode&&BootstrapUIColorMode.get())||' . json_encode($config['default']) . ';'
            . 'var ac=' . json_encode($config['activeClass']) . ';'
            . 'w.querySelectorAll("[data-bs-theme-value]").forEach(function(b){'
            . 'b.addEventListener("click",function(){if(window.BootstrapUIColorMode){BootstrapUIColorMode.set(b.getAttribute("data-bs-theme-value"));}'
            . 'w.querySelectorAll("[data-bs-theme-value]").forEach(function(x){x.classList.toggle(ac,x===b);});});'
            . 'if(b.getAttribute("data-bs-theme-value")===active){b.classList.add(ac);b.setAttribute("aria-pressed","true");}'
